import os
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"  # 解决 OMP 错误
import time
import random
from load import *
import torch
...
class Trainer:
    def __init__(self, model, record):  # 直接接受模型实例
...
def compare_models(model_names):
    all_records = {}
    for name in model_names:  # name 是字符串，如 'Original'
        print(f"Training {name}...")
        model = model_dict[name]  # 通过字符串键获取模型实例
...
def plot_comparison(all_records, metric='acc_test'):
    # 定义四个Top-K的索引和标签
    k_list = [1, 5, 10, 20]
    titles = ['Top-1 Accuracy', 'Top-5 Accuracy', 'Top-10 Accuracy', 'Top-20 Accuracy']
    
    # 创建2x2的子图布局
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    axes = axes.flatten()  # 将二维轴数组展平为一维
    
    for idx, (k_idx, title) in enumerate(zip(range(4), titles)):
        ax = axes[idx]
        for model_name, records in all_records.items():
            metrics = np.array(records[metric])
            # 提取对应Top-K的准确率数据（每个epoch的第k_idx个指标）
            k_acc = metrics[:, k_idx]
            epochs = records['epoch']
            ax.plot(epochs, k_acc, label=model_name)
            ax.set_title(title)
            ax.set_xlabel('Epoch')
    ...
    fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 1.05), ncol=len(all_records))
    
    plt.tight_layout()
    plt.savefig('model_comparison_multi_k.png')
    plt.show()
...
if __name__ == '__main__':
    ...
    emb_l = nn.Embedding(l_max + 1, 50)
    model_dict = {
    'Original': Model(t_dim=hours+1, l_dim=l_max+1, u_dim=u_max+1, embed_dim=50, ex=ex),
    'MLP': MLP(input_dim=max_len*3, hidden_dim=128, output_dim=l_max),
    'LSTM': LSTM_Model(input_dim=3, hidden_dim=64, output_dim=l_max),
    'CNN': CNN_Model(input_dim=3, output_dim=l_max),
    'SimpleAttn': SimpleAttn(l_dim=l_max+1, embed_dim=64),
    'RNN': RNN_Model(input_dim=3, hidden_dim=64, output_dim=l_max),
    'MarkovChain': MarkovChain(l_dim=l_max+1)}

    models_to_compare = ['Original', 'MLP', 'LSTM', 'CNN', 'SimpleAttn', 'RNN', 'MarkovChain']
    all_records = compare_models(models_to_compare)
    plot_comparison(all_records)
    # trainer.inference()
